...
const RemoveBackground: React.FC<{ setCurrentPage: (page: Page) => void }> = ({ setCurrentPage }) => {
  ...
  }, [files, addJobs, setCurrentPage]);

  const isProcessDisabled = files.length === 0 || !isPhotoRoomKeySet || isLoading;

  // Kiểm tra xem đang chạy ở localhost hay đã deploy (avoid SSR window usage)
